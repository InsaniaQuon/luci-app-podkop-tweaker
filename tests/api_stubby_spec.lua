...
describe("api_stubby.init_check/init_fix", function()
    ...
    it("fix: rewrites user stubby->root into tmp, logs chmod+mv and restart", function()
        H.begin({ sys = { { match = "chmod +x /etc/init.d/stubby.tmp-fix", out = "EXIT:0" } } })
    ...
    it("fix: mv failure -> error and tmp cleanup (m9)", function()
        H.begin({ sys = { { match = "chmod +x /etc/init.d/stubby.tmp-fix", out = "mv: denied\nEXIT:1" } } })
        H.vfs_write(INIT, "#!/bin/sh\n" .. MARKER .. "\n")
        local STB = H.reload("podkop-tweaker.api_stubby")
        assert.same({ error = "Cannot apply init script fix" }, STB.init_fix())
        assert.falsy(H.vfs_exists(INIT .. ".tmp-fix"))
    end)
end)
